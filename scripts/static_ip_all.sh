...
mask_to_cidr() {
    ...
    echo "$(( (o1<<24 | o2<<16 | o3<<8 | o4) ))" | awk '{
        for (i=0;i<32;i++) if (!(and($1,2^(31-i)))) break; print i;
    }'
}
...
if [ -f "$ENV_FILE" ]; then
  ...
  for iface in $interfaces; do
    ...
    if [ -f "$carrier" ] && [ "$(cat "$carrier")" -eq 1 ]; then
        ...
        dhcp_val="${!IFACE_ENV"_DHCP"}"
        dhcp_server_val="${!IFACE_ENV"_DHCP_SERVER"}"
        ip_val="${!IFACE_ENV"_IP"}"
        mask_val="${!IFACE_ENV"_MASK"}"
        gw_val="${!IFACE_ENV"_GW"}"
        dns_val="${!IFACE_ENV"_DNS"}"
        range_start="${!IFACE_ENV"_DHCP_RANGE_START"}"
        range_end="${!IFACE_ENV"_DHCP_RANGE_END"}"
...
if [ ${#dhcp_servers[@]} -gt 0 ]; then
    ...
    for entry in "${dhcp_servers[@]}"; do
        ...
        # Validação do range dentro da subnet
        net_prefix=$(echo "$network" | cut -d. -f1-3)
        start_prefix=$(echo "$start" | cut -d. -f1-3)
        if [[ "$net_prefix" != "$start_prefix" ]]; then
            echo -e "${RED}❌ $iface: IP de range $start está fora da sub-rede $network. Ignorando.${NC}"
            continue
        fi
