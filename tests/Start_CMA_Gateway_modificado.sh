...
function ler_variavel() {
    ...
    while true; do
        ...
        case "$TIPO_VALIDACAO" in
            ...
            ip_list)
                ERRO=0
                for part in $NOVO_VALOR; do
                    if ! validar_ip "$part"; then
                        echo "❌ '$part' não é um IP válido."
                        ERRO=1
                    fi
                done
                if [ $ERRO -eq 0 ]; then break; fi
                ;;
            url)
...
function configurar_interfaces_fisicas() {
    ...
    echo ""
    read -p "Digite o número da interface que deseja configurar [0-4] ou 's' para voltar: " num_iface
    if [[ "$num_iface" == "s" || "$num_iface" == "S" ]]; then
        return
