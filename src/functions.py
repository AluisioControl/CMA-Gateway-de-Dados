    ...
    try:
        ...
        for item in items:
            ...
            while (ntries + 1 > current_try):
                ...
                if check_rabbitmq_connection(): #check_rabbitmq_connection(host='localhost', username='cma_gateway', password='cm@GTW'):
...
# --------------------------------------------------------------------
# Função que coleta "xid_equip" com os tempos e tipos de intervalo
# da tabela de sensores Modbus ou DNP3
# --------------------------------------------------------------------
...
def send_to_mqtt_broker(valor, protocol):
    if valor != "null":
        ...
        #print( send_to_mqtt_broker = ", "valor:", valor , "protocol:", protocol, "payload_mqtt:", payload_mqtt)
...
# -------------------------------------------------------------
# thread de verificação de status do servidor
# -------------------------------------------------------------
...
# -------------------------------------------------------------
# Thread de mapeamento individual de tempo de envio 
# para sensores Modbus e DNP3
# -------------------------------------------------------------
def thr_start_routines_sensor(datasource, protocol):
    ...
    while True:
        #time_list = get_periods_func(datasource) 
...
#-------------------------------------
# TODO: 
# (OK) Fazer script shell de configuração geral com menus e submenus (salvar config.ini)
# Pegar as configurações do config.ini
# (OK) logger nas mensagens de erro
